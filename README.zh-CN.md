...
v0.2.0 通过同一套 Project、Job、Step、Segment 与 Artifact 模型提供三条链路：

- TXT → 段落感知分段 → Mock 或 OpenAI-compatible 翻译 → QA → TXT/JSON；
- 未加密 EPUB 2/3 → 保留结构的可见文本抽取 → 翻译与 QA → 经验证的 EPUB；
- 图片/CBZ → 按能力选择漫画 Adapter → 译后页面与原始输出 → CBZ。

EPUB 链路保留阅读顺序、导航、链接、锚点、封面、图片、CSS、字体和其他非文本资源。
翻译缺失或失败的文本按明确规则回退为不可变源文本。精确的文本节点规则、重建验证、安全
限制与已知限制见 [EPUB 支持文档](docs/epub.md)。
...
EPUB 使用同一组命令；源文件类型会自动选择 EPUB Pipeline，也可显式指定：
...
`--output` 以流式文件复制方式写出不可变 Artifact；默认拒绝覆盖已有路径，需显式提供
`--overwrite`。

CLI 成功时退出码为 `0`；doctor/可用性检查未通过时为 `1`；配置或应用错误输出统一 JSON
错误信封并以 `2` 退出，便于脚本判断。
...
上传 EPUB 时将 `source` 换成
`source=@book.epub;type=application/epub+zip`。上传在配置上限内流式写入 Artifact Store；
Artifact 下载使用文件响应，不会先把整个文件读入应用内存。
...
实际执行结果以[分版本验收归档](acceptance/README.md)为准；v0.2.0 在验收报告审阅前不会
自动打 tag 或发布 Release。核心代码采用
