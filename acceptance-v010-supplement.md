...
- Git commit：`Pending`——真实 Provider 与回归门禁已通过，正在建立首个可审计 Git 基线
...
| Git | `main`; baseline commit pending at this report stage |
...
- 首个 Git commit/tag/Release 在仓库审计完成前仍待创建。
...
命令已通过，不要将 Codex 沙箱的 socket 权限错误记为 WSL integration 故障。本报告阶段尚未创建
tag/GitHub Release，且没有开始 v0.2.0。
