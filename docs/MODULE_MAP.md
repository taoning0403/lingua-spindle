...
| `docs/PRODUCT_SPEC.md` | Durable v0.3.1 product and acceptance contract. |
...
| `docs/migrations/v0.2-to-v0.3.md`, `v0.3-to-v0.3.1.md` | Full-data-root backup, additive migrations 0003/0004, verification, and restore rollback. |
...
| `docs/releases/v0.1.0.md` through `v0.3.1.md` | Versioned highlights, security, upgrade, and limitations. |
...
| `acceptance/v0.3.1/` | Checksummed service-hardening archive with exact Wheel, migration/idempotency/security/Docker evidence, command log, environment, and deterministic samples. |
| `tools/generate_v031_acceptance.py` | v0.3.1 entry for the deterministic versioned TXT/EPUB/manga sample generator. |
| `tools/verify_v031_extras.py` | v0.3.1 entry for exact-commit isolated core/openai/manga/runtime/cli/server/all Wheel verification. |
...
| `database.py`, `migrations/` | SQLite and atomic forward-only migrations 0001–0004. | `test_database_migrations.py` |
| `models.py` | Private SQLAlchemy records; migration 0004 adds Job fingerprints/request IDs and durable idempotency claims. | migration/runtime tests |
| `idempotency.py` | Key/request-ID validation, immediate key hashing, canonical request fingerprints, and service result metadata. | `tests/unit/test_idempotency.py` |
| `storage.py` | Private safe immutable Artifact payload store and atomic publication/copy. | storage/Artifact tests |
| `application.py` | Optional Project/Profile/Job/Artifact use cases, durable idempotency, active Job coalescing, and core DTO mapping. | integration/interface tests |
...
| `interfaces/api.py` | JSON/OpenAPI-only persistent API, request correlation/idempotency protocol, bounded upload/download, errors, and headless root. | `[server]`; OpenAPI/API tests |
...
| `tests/integration/test_database_migrations.py` | Fresh and v0.1/v0.2/v0.3-compatible forward migration through schema 0004. |
...
| `tests/unit/test_idempotency.py`, `tests/integration/test_service_idempotency.py` | Canonical fingerprints; durable replay/conflict/restart; two-instance Project/Job concurrency; Provider protection; forced/compatibility mode; request correlation. |
...
| Path | Responsibility |
| --- | --- |
| `pyproject.toml`, `constraints-v031.txt` | Minimal default dependency, extras, entry point, static gates, and current direct-version constraints. Historical constraint files stay versioned. |
...
openai, manga, runtime, cli, server, and all. Docker start requires a working engine. The v0.3.1
